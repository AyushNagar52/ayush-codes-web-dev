...
/**
 * Executes a Market BUY Order (Supports both ReplicaSet Transactions and Standalone Dev MongoDB)
 */
const executeBuyOrder = async (userId, symbol, quantity) => {
  ...
  // 2. Find portfolio and verify cash balance
  ...
  // 4. Query or Create Holding with Weighted Average Cost (WAC)
...
const executeSellOrder = async (userId, symbol, quantity) => {
  ...
  // 2. Find holding and verify quantity
  const holding = await Holding.findOne({ userId, symbol: cleanSymbol });
